...
namespace MyGame
{
	public class SpreadQuestionScreen : QuestionScreen
	{
		...
		private TextSpreader _txtSpread;
		...
		private void OnButtonClick(object sender, ButtonClickArgs e) {
			ScreenManager.UserAnswers.Add (RefVal + ": " + e.Text.ToLower ());
			if (e.Text.ToLower () == _currentCorrectAnswer)
			{
				SM.CorrectSpreadAnswers++;
			}
			CurrentStage++;
		}
		...
		public SpreadQuestionScreen (ScreenManager sm, string text, Dictionary<string, List<string>> questions, int time, Color backColor, Color foreColor, int val) : base(sm, text, val) {
			this.questions = questions;
			_backColor = backColor;
			_foreColor = foreColor;
			_txtSpread = new TextSpreader (text, time);
			_txtSpread.Finished += OnTextFinish;
		...
		public SpreadQuestionScreen (ScreenManager sm, string text, Dictionary<string, List<string>> questions, int time, int val) : this(sm, text, questions, time, Color.White, Color.Black, val) { }
	}
